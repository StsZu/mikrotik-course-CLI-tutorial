  ...
  lessons: [
    {
      ...
      steps: [
        ...
        { type: "cli", title: "Перевірка IP-рівня",
          commands: [
            { cmd: "/ip route print", explain: "Маршрути. <code>0.0.0.0/0</code> — маршрут за замовчуванням в інтернет; <code>DAd</code> — динамічний, активний, отриманий від DHCP.", output: "Flags: D - DYNAMIC; I - INACTIVE, A - ACTIVE; c - CONNECT, s - STATIC, d - DHCP\nColumns: DST-ADDRESS, GATEWAY, DISTANCE\n#     DST-ADDRESS      GATEWAY       DISTANCE\n0 DAd 0.0.0.0/0        198.51.100.1         1\n1 DAc 10.0.0.0/24      bridge               0\n2 DAc 198.51.100.0/24  ether1               0", risk: "low" },
        ...
        { type: "terminal", title: "Спробуй: маршрут в інтернет",
          ...
          output: "Flags: D - DYNAMIC; I - INACTIVE, A - ACTIVE; c - CONNECT, s - STATIC, d - DHCP\nColumns: DST-ADDRESS, GATEWAY, DISTANCE\n#     DST-ADDRESS  GATEWAY       DISTANCE\n0 DAd 0.0.0.0/0    198.51.100.1         1",
        ...
        { type: "cli", title: "Перші зміни: add і set",
          commands: [
            { cmd: "/ip address add address=10.0.1.1/24 interface=ether5", explain: "Нова мережа <code>10.0.1.0/24</code> на лабораторному порту. Роутер сам додасть connected-маршрут <code>10.0.1.0/24</code> (прапорець <code>c</code>). Активним (<code>DAc</code>) він буде, лише коли ether5 увімкнений і має лінк; інакше — неактивний <code>DIc</code>.", risk: "medium" },
      ...
      quiz: [
        ...
        { question: "Що станеться після `/ip address add address=10.0.1.1/24 interface=ether5`?", options: ["На ether5 з'явиться адреса, а в маршрутах — connected-маршрут `10.0.1.0/24`", "Зміниться адреса LAN", "Нічого, це перегляд"], correct: 0, feedback: "Connected-маршрут (`c`) з'являється автоматично для кожної адреси. Активним (`A`) він стає, коли інтерфейс увімкнений і має лінк; якщо ether5 вимкнений — маршрут буде `DIc` (неактивний)." },
